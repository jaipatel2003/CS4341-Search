...
class Queue:
    ...
    def sortByCost(self):
        ...
        self.nodes = sorted(self.nodes, key=lambda node: (
        node.cost, node.path[0].name, len(node.path), (list(map(lambda state: state.name, node.path)))))
    ...
    def removeRedundantNodes(self):
        ...
        for index in range(len(self.nodes)):
            ...
            if nodeName in bestSeen and self.nodes[index] != bestSeen[nodeName] and self.nodes[index].cost >= \
                    bestSeen[nodeName][0].cost:
