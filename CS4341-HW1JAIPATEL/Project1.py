...
def General_Search(problem, search):
    ...
    queue = Queue.Queue(
        Queue.Node([problem.getState(initialState)], costFunctions[search]([problem.getState(initialState)])))
    explored = []
...
def expand_queue(queue, newNodesToAddToQueue, problem, search):
    ...
    if search == SearchEnum.DEPTH_FIRST_SEARCH:
        ...
        """
        putin = []
        for node in newNodesToAddToQueue:
            if len(node.path) <= lengthLimit:
                putin.append(node)

        return queue.insertNodesAtFront(putin)
        ...
        if (iterativeLimit == 0):
            return iterativeDeepening(
                problem)  # Need to return the node here because of the nested General_Search calls in the iterativeDeepening function.
        else:
            """
            Include your solution for Depth-limited here. Hint: 1 line of code (a call to a function provided) sufficies, but you can have more lines if you need to.
            """
            putin = []
            for node in newNodesToAddToQueue:
                if len(node.path) <= iterativeLimit+1:  # +1 for root
                    putin.append(node)
            return queue.insertNodesAtFront(putin)
        ...
        """
        #queue.insertNodesAtEnd(newNodesToAddToQueue)
        #queue.removeRedundantNodes() #3333333333333333333333333333333333333
        #queue.keepBestNodes(2)
        #if queue.areNodePathLengthsEqual() == True:
        #    queue.keepBestNodes(2)
        #else:
        #    queue.keepBestNodes(2)
        queue.insertNodesAtEnd(newNodesToAddToQueue)
